...
class ModelRawMats{
	static public function mdlAddRawMats($data){
		$db = new Connection();
		$pdo = $db->connect();
        try{
            ...
            $item_id = $pdo->prepare("SELECT CONCAT(LPAD((count(id)+1),5,'0')) as gen_id  FROM rawmats FOR UPDATE");
			...
			$stmt->bindParam(":critical", $data["critical"], PDO::PARAM_STR);
			$stmt->bindParam(":low", $data["low"], PDO::PARAM_STR);
			$stmt->bindParam(":moderate", $data["moderate"], PDO::PARAM_STR);
			$stmt->bindParam(":high", $data["high"], PDO::PARAM_STR);
			$stmt->execute();
	...
	static public function mdlEditRawMats($data){
		$db = new Connection();
		$pdo = $db->connect();
        try{
        	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			...
			$stmt = $pdo->prepare("UPDATE rawmats SET itemid = :itemid,
															 pdesc = :pdesc,
	...
	static public function mdlShowAllRawMats(){
		$stmt = (new Connection)->connect()->prepare("SELECT * FROM rawmats ORDER BY pdesc");
		$stmt -> execute();
	...
	static public function mdlShowRawMatsList($categorycode){
		if ($categorycode != ''){
		...
		$stmt = (new Connection)->connect()->prepare("SELECT a.itemid,a.itemcode,b.catdescription,a.pdesc,a.ucost FROM rawmats AS a INNER JOIN categoryrawmats AS b ON (a.categorycode = b.categorycode) $whereClause ORDER BY a.pdesc");
	...
	static public function mdlShowRawMat($item, $value){
		$stmt = (new Connection)->connect()->prepare("SELECT * FROM rawmats WHERE $item = :$item");
	...
	static public function mdlShowPurchaseRawMatProducts(){
		$stmt = (new Connection)->connect()->prepare("SELECT * FROM rawmats WHERE (isactive = 1) ORDER BY pdesc");
		$stmt -> execute();
		return $stmt -> fetchAll();
		$stmt -> close();
		$stmt = null;
	...
	static public function mdlProductComponentsList(){
		$stmt = (new Connection)->connect()->prepare("SELECT * FROM rawmats WHERE (isactive = 1) AND ((classification = 'Component') OR (categorycode = '0005')) ORDER BY pdesc");
		$stmt -> execute();
		return $stmt -> fetchAll();
		$stmt -> close();
		$stmt = null;
	...
	static public function mdlWasteDamageList(){
		$stmt = (new Connection)->connect()->prepare("SELECT * FROM rawmats WHERE (isactive = 1) AND (classification = 'Recycle') ORDER BY pdesc");
		$stmt -> execute();
		return $stmt -> fetchAll();
		$stmt -> close();
		$stmt = null;
	...
	static public function mdlPurchaseItemsList(){
		$stmt = (new Connection)->connect()->prepare("SELECT * FROM rawmats WHERE (isactive = 1) AND (purchaseitem = 1) ORDER BY pdesc");
		$stmt -> execute();
		return $stmt -> fetchAll();
		$stmt -> close();
		$stmt = null;
	...
	// Get raw material details using itemid
	static public function mdlShowTransactionRawMat($itemid){
		$stmt = (new Connection)->connect()->prepare("SELECT a.itemid,a.itemcode,b.catdescription,a.pdesc,a.ucost,a.meas2,a.critical,a.low,a.moderate,a.high,a.classification FROM rawmats AS a INNER JOIN categoryrawmats AS b ON (a.categorycode = b.categorycode) WHERE (a.itemid = '$itemid')");
		$stmt -> execute();
		return $stmt -> fetch();
		$stmt -> close();
		$stmt = null;
	...
	// static public function mdlRawMatReport($reptype, $filter, $categorycode){
	// 	if ($categorycode != ''){
	// 		$category = " AND (a.categorycode = '$categorycode')";
	// 	}else{
	// 		$category = "";
