...
if (!$.fn.DataTable.isDataTable('.rawoutTransactionTable')) {
  var pt = $('.rawoutTransactionTable').DataTable({
      deferRender: true,
      processing: true,
      autoWidth: true,
      scrollY: 360,
...
$(function() {
 ...
 // When browsing Purchase Order - set Status to Pending | Partial
  ...
  $("#btn-search").click(function(){
    $(".rawoutTransactionTable").DataTable().clear();
    pt.draw();
  ...
  $('#lst-machineid, #lst_date_range, #lst-empid, #lst-reqstatus').on("change", function(){
    ...
    $.ajax({
        url:"ajax/rawout_transaction_list.ajax.php",   
        method: "POST",                
        data: data,                    
        cache: false,                  
        contentType: false,            
        ...
        success:function(answer){
              ...
              for(var i = 0; i < answer.length; i++) {
                ...
                var button = "<td><button type='button' class='btn btn-outline btn-sm bg-green-400 border-green-400 text-green-400 btn-icon rounded-round border-2 ml-2 btnEditRawout' reqnumber='"+reqnumber+"'><i class='icon-pencil3'></i></button></td>";  
                pt.row.add([reqdate, requestor, reqnumber, shift, machine, total_amount, button]); 
              }
              pt.draw();
  ...
  $(".rawoutTransactionTable tbody").on('click', '.btnEditRawout', function () {
    var reqnumber = $(this).attr("reqnumber");
    var data = new FormData();
    data.append("reqnumber", reqnumber);
    $.ajax({
      url:"ajax/rawout_get_record.ajax.php",
      method: "POST",
      data: data,
      cache: false,
      contentType: false,
      ...
      success:function(answer){
          ...
          $.ajax({
            url:"ajax/rawout_get_items.ajax.php",
            method: "POST",
            data: data_items,
            cache: false,
            contentType: false,
            ...
            success:function(products){
              for(var p = 0; p < products.length; p++) {
                ...
                $(".enlisted_products").append(
                    ...
                    '<td class="mcEntry" width="10%" style="padding:2px; text-align:center;">'+
                        '<input type="checkbox" class="matcost" name="matcost" itemid="'+itemid+'" value="'+matcost+'" style="transform: scale(1.5); border: 2px solid white; padding: 5px;" ' + (matcost == 1 ? 'checked' : '') + '>'+
                    '</td>' +
          ...
          $("#modal-search-rawout").modal('hide');
      }
    });
  });  
        ...
        success:function(answer){
          ...
          var price = numberWithCommas(price_amount);
           ...
           $(".enlisted_products").append(
            ...
            // Modified Checkbox Column (New 2nd Column)
            // '<td class="mcEntry" width="10%" style="padding:2px; text-align:center;">'+
            //     '<input type="checkbox" class="matcost" name="matcost" itemid="'+itemid+'" style="transform: scale(1.5); border: 2px solid white; padding: 5px;">'+
            // '</td>' +
  ...
  // Listen for checkbox change and update matcost value
// $(".transactionProductsTable").on("change", ".matcost", function(){
//     listItems(); 
//     alert($("#productList").val());
// });

  $(".rawout-form").on("change", "input.matcost", function(){
      var itemid = $(this).parent().parent().children(".mcEntry").children().attr("itemid");
      listItems(); 
      // alert($("#productList").val());
  })

  // Input QTY or PRICE
  $(".rawout-form").on("keydown keypress blur focus", "input.qty,input.price", function(){
  ...
  $(".rawout-form").on("keydown keypress blur focus", "input.qty", function(){
      var itemid = $(this).parent().parent().children(".qtyEntry").children().attr("itemid");
      _gblBindNumericClasses('numeric'); 
      listItems(); 
  })
  ...
  $(".rawout-form").on("click", "button.removeProduct", function(){
  ...
  function listItems(){
    ...
    var priceamount = $(".price");
    var totalamount = $(".tamount");
    var checkboxes = $(".matcost"); // Get all checkboxes
    ...
    if (num_entries > 0){
        for(var i = 0; i < num_entries; i++){
            ...
            var txt_price = $(priceamount[i]).val();
            var txt_tamount = $(totalamount[i]).val();
            var checkboxValue = $(checkboxes[i]).prop("checked") ? 1 : 0; // If checked, 1; if unchecked, 0
            ...
            productList.push({
                ...
                "tamount" : tamount.toFixed(2),
                "itemid" : $(description[i]).attr("itemid"),
                "matcost": checkboxValue // Store 1 if checked, 0 if not
            })      
        }
  ...
  function initialize(){
    $("#sel-machineid").val('').trigger('change');
    $("#sel-requestby").val('').trigger('change');
    $("#txt-reqnumber").val("");
    $("#txt-reqstatus").val("Posted");
    $("#tns-remarks").val("");
    $("#num-amount").val("0.00");
  ...
  $(".rawout-form").submit(function (e) {
      e.preventDefault();
      swal.fire({
          title: 'Do you want to save raw material requisition?',
          type: 'question',
          ...
          if(result.value) {
            ...
            var rawwithdraw = new FormData();
            rawwithdraw.append("trans_type", trans_type);
            rawwithdraw.append("machineid", machineid);
            rawwithdraw.append("requestby", requestby);
            rawwithdraw.append("reqdate", reqdate);
            rawwithdraw.append("reqnumber", reqnumber);
            rawwithdraw.append("shift", shift);
            rawwithdraw.append("reqstatus", reqstatus);
            rawwithdraw.append("remarks", remarks);
            rawwithdraw.append("postedby", postedby);
            rawwithdraw.append("productlist", productlist);
           
            $.ajax({
               url:"ajax/rawout_save_record.ajax.php",
               method: "POST",
               data: rawwithdraw,
               cache: false,
               contentType: false,
               ...
               complete: function () {
                swal.fire({
                    title: 'Raw material requisition successfully saved!',
                    type: 'success',
                    allowOutsideClick: false,
   ...
   $("#btn-cancel").click(function(){
      swal.fire({
          ...
          if(result.value) {
            var reqnumber = $("#txt-reqnumber").val();           
            var cancelrequest = new FormData();
            cancelrequest.append("reqnumber", reqnumber);            
            $.ajax({
               url:"ajax/rawout_cancel_record.ajax.php",
               method: "POST",
               data: cancelrequest,
               cache: false,
               contentType: false,
