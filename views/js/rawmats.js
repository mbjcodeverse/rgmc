...
if (!$.fn.DataTable.isDataTable('.rawmatsTable')) {
  var rw = $('.rawmatsTable').DataTable({
      deferRender: true,
      processing: true,
      autoWidth: true,
      scrollY: 360,
...
$(function() {
   $(".select").select2({
    ...
    $("#lbl-classification").click(function(){
      $("#sel-classification").val('').trigger('change');
   }); 

   $(".select-search").select2();

   $('#rawmats-form input[id^="num"]').on("keypress", function (e) {
      return _helper.isNumericDash(e) ? true : e.preventDefault();
   });

   $('#rawmats-form input[id^="txt"]').on("keypress", function (e) {
      return _helper.isString(e) ? true : e.preventDefault();
   });

   $('#rawmats-form input[id^="tns"]').on("keypress", function (e) {
   ...
   // Items Details
   $(".rawmats-form").submit(function (e) {
      e.preventDefault();

      if ($('#trans_type').val() == 'New'){
          var title = "Do you want to save new raw material details?";
      }else{
          var title = "Do you want to update raw material details?";
      }
      ...
      swal.fire({
          ...
          if(result.value) {
            ...
            var txt_low = $("#num-low").val();
            var low = parseFloat(txt_low.replaceAll(",",""));  

            var txt_moderate = $("#num-moderate").val();
            var moderate = parseFloat(txt_moderate.replaceAll(",",""));  

            var txt_high = $("#num-high").val();
            var high = parseFloat(txt_high.replaceAll(",","")); 
            ...
            items.append("critical", critical);  
            items.append("low", low);  
            items.append("moderate", moderate);  
            items.append("high",high);
            $.ajax({
               url:"ajax/rawmats_save_record.ajax.php",
               method: "POST",
               data: items,
               cache: false,
               contentType: false,
               ...
               complete: function () {
                 swal.fire({
                    title: 'Raw material save successfully!',
                    type: 'success',
   ...
   $('#lst-categorycode').on("change", function(){
      ...
      $.ajax({
           url:"ajax/rawmats_list.ajax.php",   
           method: "POST",                
           data: data,                    
           cache: false,                  
           contentType: false,            
           ...
           success:function(answer){
                ...
                for(var i = 0; i < answer.length; i++) {
                  ...
                  rw.row.add([catdescription, itemcode, pdesc, ucost, button]); 
                }
                rw.draw();
   ...
   $('#modal-search-rawmats').on('shown.bs.modal', function () {
        $('#lst-categorycode').val('').trigger('change');

        // Clear DataTable filter (search box)
        if ($.fn.DataTable.isDataTable('.rawmatsTable')) {
            $('.rawmatsTable').DataTable().search('').draw();
        }
   });

   $(".rawmatsTable tbody").on('click', '.btnEditProduct', function () {
      var itemid = $(this).attr("itemid");
      var data = new FormData();
      data.append("itemid", itemid);
      $.ajax({
       url:"ajax/rawmats_get_record.ajax.php",
         method: "POST",
         data: data,
         cache: false,
         contentType: false,
         ...
         success:function(answer){
            ...
            $("#sel-classification").val(answer["classification"]).trigger('change');
            $("#num-critical").val(numberWithCommas(answer["critical"]));
            $("#num-low").val(numberWithCommas(answer["low"]));
            $("#num-moderate").val(numberWithCommas(answer["moderate"]));
            $("#num-high").val(numberWithCommas(answer["high"]));

            $("#trans_type").val("Update");
            $("#modal-search-rawmats").modal('hide');
        }
      })
   });        
