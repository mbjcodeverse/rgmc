...
if (!$.fn.DataTable.isDataTable('.matreturnTransactionTable')) {
  var pt = $('.matreturnTransactionTable').DataTable({
      deferRender: true,
      processing: true,
      autoWidth: true,
      scrollY: 360,
...
$(function() {
 ...
 // When browsing Purchase Order - set Status to Pending | Partial
  ...
  $("#btn-search").click(function(){
    $(".matreturnTransactionTable").DataTable().clear();
    pt.draw();
  ...
  $('#lst-machineid, #lst_date_range, #lst-empid, #lst-retstatus, #lst-returntype').on("change", function(){
    ...
    $.ajax({
        url:"ajax/matreturn_transaction_list.ajax.php",   
        method: "POST",                
        data: data,                    
        cache: false,                  
        contentType: false,            
        ...
        success:function(answer){
              ...
              for(var i = 0; i < answer.length; i++) {
                ...
                let sw = answer[i];
                let req_date = sw.retdate;
                let retdate = req_date.split("-");
                retdate = retdate[1] + "/" + retdate[2] + "/" + retdate[0];

                let returnby = sw.return_by;
                let retnumber = sw.retnumber;
                let shift = sw.shift;
                let machinedesc = sw.machinedesc;
                let machabbr = sw.machabbr;
                let retstatus = sw.retstatus;
                // let returntype = sw.returntype;
                ...
                var button = "<td><button type='button' class='btn btn-outline btn-sm bg-green-400 border-green-400 text-green-400 btn-icon rounded-round border-2 ml-2 btnEditMatreturn' retnumber='"+retnumber+"'><i class='icon-pencil3'></i></button></td>";  
                pt.row.add([retdate, returnby, retnumber, shift, machine, total_amount, button]); 
              }
              pt.draw();
  ...
  $(".matreturnTransactionTable tbody").on('click', '.btnEditMatreturn', function () {
    var retnumber = $(this).attr("retnumber");
    var data = new FormData();
    data.append("retnumber", retnumber);
    $.ajax({
      url:"ajax/matreturn_get_record.ajax.php",
      method: "POST",
      data: data,
      cache: false,
      contentType: false,
      ...
      success:function(answer){
          ...
          $("#sel-returntype").val(answer["returntype"]).trigger('change');
          ...
          $.ajax({
            url:"ajax/matreturn_get_items.ajax.php",
            method: "POST",
            data: data_items,
            cache: false,
            contentType: false,
          ...
          $("#modal-search-matreturn").modal('hide');
      }
    });
  });  
  ...
  // Input QTY or PRICE
  $(".matreturn-form").on("keydown keypress blur focus", "input.qty,input.price", function(){
  ...
  $(".matreturn-form").on("keydown keypress blur focus", "input.qty", function(){
      var itemid = $(this).parent().parent().children(".qtyEntry").children().attr("itemid");
      _gblBindNumericClasses('numeric'); 
      listItems(); 
  })
  ...
  $(".matreturn-form").on("click", "button.removeProduct", function(){
  ...
  function initialize(){
    $("#sel-machineid").val('').trigger('change');
    $("#sel-returnby").val('').trigger('change');
    $("#sel-returntype").val('').trigger('change');
    $("#txt-retnumber").val("");
    $("#txt-retstatus").val("Posted");
    $("#tns-remarks").val("");
    $("#num-amount").val("0.00");
  ...
  $(".matreturn-form").submit(function (e) {
      e.preventDefault();
      swal.fire({
          title: 'Do you want to save returned materials?',
          type: 'question',
          ...
          if(result.value) {
            ...
            let returntype = $("#sel-returntype").val();
            ...
            let retnumber = $("#txt-retnumber").val();
            ...
            rawwithdraw.append("returnby", returnby);
            rawwithdraw.append("returntype", returntype);
            rawwithdraw.append("retdate", retdate);
            rawwithdraw.append("retnumber", retnumber);
            rawwithdraw.append("shift", shift);
            rawwithdraw.append("retstatus", retstatus);
            rawwithdraw.append("remarks", remarks);
            rawwithdraw.append("postedby", postedby);
            rawwithdraw.append("productlist", productlist);
           
            $.ajax({
               url:"ajax/matreturn_save_record.ajax.php",
               method: "POST",
               data: rawwithdraw,
               cache: false,
               contentType: false,
               ...
               complete: function () {
                swal.fire({
                    title: 'Returned materials successfully saved!',
                    type: 'success',
                    allowOutsideClick: false,
   ...
   $("#btn-cancel").click(function(){
      swal.fire({
          title: 'Do you want to cancel returned materials?',
          ...
          if(result.value) {
            var retnumber = $("#txt-retnumber").val();           
            var cancelrequest = new FormData();
            cancelrequest.append("retnumber", retnumber);            
            $.ajax({
               url:"ajax/matreturn_cancel_record.ajax.php",
               method: "POST",
               data: cancelrequest,
               cache: false,
               contentType: false,
