$(function() {
     ...
     $('#lst-machineid, #lst_date_range, #lst-categorycode, #lst-postedby, #lst-operatedby, #lst-excstatus, #lst-reptype').on("change", function(){
        ...
        // alert(machineid + ' ' + start_date + ' ' + end_date + ' ' + postedby + ' ' + operatedby + ' ' + excstatus + ' ' + reptype);
        ...
        $.ajax({
             url:"ajax/excess_report.ajax.php",   
             method: "POST",                
             data: data,                    
             cache: false,                  
             contentType: false,            
                      ...
                      html.push('<div class="table-responsive" style="overflow-y: auto; max-height: 470px;">');
                        html.push('<table class="table mx-auto w-auto">');
                          ...
                          for(var i = 0; i < answer.length; i++) {
                              ...
                              var exc_date = release.excdate;
                              var excdate = exc_date.substring(5, 7) + '/' + exc_date.substring(8, 10) + '/' + exc_date.substring(0, 4);
  
                              var excnumber = release.excnumber;
                              var machabbr = release.machabbr;
                              var machinedesc = release.machinedesc;
                              var excstatus = release.excstatus;
                              var name = release.name;
                              var oprname = release.oprname;
                              ...
                              if (prodname == null){
                                ...
                                if (i == 0){
                                  var prev_excnumber = release.excnumber;
                                  var prev_excdate = release.excdate;
                                }else{
                                  var curr_excnumber = release.excnumber;
                                  if (prev_excnumber == curr_excnumber){
                                    excnumber = '';
                                    machine = '';
                                    name = '';
                                    oprname = '';
                                  }
                                  var prev_excnumber = curr_excnumber;
                                  // don't display same date
                                  var curr_excdate = release.excdate;
                                  if (prev_excdate == curr_excdate){
                                    excdate = '';
                                  }
                                  var prev_excdate = curr_excdate;                    
                                }                 
                              }
  
                              html.push('<tr>');
                                html.push('<td>'+excdate+'</td>');
  
                                if (excstatus == 'Void'){
                                  html.push('<td style="color:orange;">'+excnumber+'</td>');
                                }else{
                                  html.push('<td>'+excnumber+'</td>');
                                }
