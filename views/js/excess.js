...
if (!$.fn.DataTable.isDataTable('.excessTransactionTable')) {
  var pt = $('.excessTransactionTable').DataTable({
      deferRender: true,
      processing: true,
      autoWidth: true,
      scrollY: 360,
...
$(function() { 
 ...
 // When browsing Purchase Order - set Status to Pending | Partial
  ...
  $("#btn-search").click(function(){
    $(".excessTransactionTable").DataTable().clear();
    pt.draw();
    $('#lst-excstatus').val('Posted').trigger('change');
  });   
  ...
  $("#lbl-lst-excstatus").click(function(){
      $("#lst-excstatus").val('').trigger('change');
  });      
  ...
  $('#lst-machineid, #lst_date_range, #lst-empid, #lst-excstatus').on("change", function(){
    ...
    let excstatus = $("#lst-excstatus").val();
    ...
    data.append("excstatus", excstatus);

    $.ajax({
        url:"ajax/excess_transaction_list.ajax.php",   
        method: "POST",                
        data: data,                    
        cache: false,                  
        contentType: false,            
        ...
        success:function(answer){
              ...
              for(var i = 0; i < answer.length; i++) {
                ...
                let sw = answer[i];
                let req_date = sw.excdate;
                let excdate = req_date.split("-");
                excdate = excdate[1] + "/" + excdate[2] + "/" + excdate[0];

                let operatedby = sw.operated_by;
                let excnumber = sw.excnumber;
                let shift = sw.shift;
                let machinedesc = sw.machinedesc;
                let machabbr = sw.machabbr;
                let excstatus = sw.excstatus;
                ...
                var button = "<td><button type='button' class='btn btn-outline btn-sm bg-green-400 border-green-400 text-green-400 btn-icon rounded-round border-2 ml-2 btnEditExcess' excnumber='"+excnumber+"'><i class='icon-pencil3'></i></button></td>";  
                pt.row.add([excdate, operatedby, excnumber, shift, machine, total_amount, button]); 
              }
              pt.draw();
  ...
  $(".excessTransactionTable tbody").on('click', '.btnEditExcess', function () {
    var excnumber = $(this).attr("excnumber");
    var data = new FormData();
    data.append("excnumber", excnumber);
    $.ajax({
      url:"ajax/excess_get_record.ajax.php",
      method: "POST",
      data: data,
      cache: false,
      contentType: false,
      ...
      success:function(answer){
          ...
          let excdate = answer["excdate"].split("-");
          excdate = excdate[1] + "/" + excdate[2] + "/" + excdate[0];
          if (excdate == '00/00/0000'){      
            excdate = '';
          }
          $("#date-excdate").val(excdate);
          $("#txt-excstatus").val(answer["excstatus"]);
          $("#txt-excnumber").val(answer["excnumber"]);
          ...
          data_items.append("excnumber", excnumber);
          $.ajax({
            url:"ajax/excess_get_items.ajax.php",
            method: "POST",
            data: data_items,
            cache: false,
            contentType: false,
          ...
          let excstatus = $("#txt-excstatus").val();
          switch (excstatus){
            case 'Cancelled':
          ...
          $("#modal-search-excess").modal('hide');
      }
    });
  });  
  ...
  // Input QTY or PRICE
  $(".excess-form").on("keydown keypress blur focus", "input.qty,input.price", function(){
  ...
  $(".excess-form").on("keydown keypress blur focus", "input.qty", function(){
      var itemid = $(this).parent().parent().children(".qtyEntry").children().attr("itemid");
      _gblBindNumericClasses('numeric'); 
      listItems(); 
  })
  ...
  $(".excess-form").on("click", "button.removeProduct", function(){
  ...
  $(".excess-form").submit(function (e) {
      e.preventDefault();
      swal.fire({
          title: 'Do you want to save excess materials?',
          type: 'question',
          ...
          if(result.value) {
            ...
            let format_excdate = $("#date-excdate").val().split("/");
            format_excdate = format_excdate[2] + "-" + format_excdate[0] + "-" + format_excdate[1];
            
            let excdate = format_excdate;
            let excnumber = $("#txt-excnumber").val();
            ...
            var excessmaterials = new FormData();
            excessmaterials.append("trans_type", trans_type);
            excessmaterials.append("machineid", machineid);
            excessmaterials.append("operatedby", operatedby);
            excessmaterials.append("excdate", excdate);
            excessmaterials.append("excnumber", excnumber);
            excessmaterials.append("shift", shift);
            excessmaterials.append("excstatus", excstatus);
            excessmaterials.append("remarks", remarks);
            excessmaterials.append("postedby", postedby);
            excessmaterials.append("productlist", productlist);
           
            $.ajax({
               url:"ajax/excess_save_record.ajax.php",
               method: "POST",
               data: excessmaterials,
               cache: false,
               contentType: false,
               processData: false,
               dataType:"text",
               ...
               complete: function () {
                swal.fire({
                    title: 'Excess materials successfully saved!',
                    type: 'success',
                    allowOutsideClick: false,
   ...
   $("#btn-cancel").click(function(){
      swal.fire({
          title: 'Do you want to Cancel excess materials?',
          ...
          if(result.value) {
            var excnumber = $("#txt-excnumber").val();           
            var cancelrequest = new FormData();
            cancelrequest.append("excnumber", excnumber);            
            $.ajax({
               url:"ajax/excess_cancel_record.ajax.php",
               method: "POST",
               data: cancelrequest,
               cache: false,
               contentType: false,
