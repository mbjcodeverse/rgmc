...
if (!$.fn.DataTable.isDataTable('.prodcomTransactionTable')) {
  var pt = $('.prodcomTransactionTable').DataTable({
      deferRender: true,
      processing: true,
      autoWidth: true,
      scrollY: 360,
...
$(function() {
  ...
  $("#btn-search").click(function(){
    $(".prodcomTransactionTable").DataTable().clear();
    pt.draw();
  ...
  // Search Production Component - Modal Form dynamic selector
  $('#lst-machineid, #lst_date_range, #lst-empid, #lst-prodstatus').on("change", function(){
    ...
    $.ajax({
        url:"ajax/prodcom_transaction_list.ajax.php",   
        method: "POST",                
        data: data,                    
        cache: false,                  
        contentType: false,            
  ...
  // Get Stock Production Component
  $(".prodcomTransactionTable tbody").on('click', '.btnEditProdcom', function () {
    var prodnumber = $(this).attr("prodnumber");
    var data = new FormData();
    data.append("prodnumber", prodnumber);
    $.ajax({
      url:"ajax/prodcom_get_record.ajax.php",
      method: "POST",
      data: data,
      cache: false,
      contentType: false,
      ...
      success:function(answer){
          ...
          $.ajax({
            url:"ajax/prodcom_get_items.ajax.php",
            method: "POST",
            data: data_items,
            cache: false,
            contentType: false,
          ...
          $("#modal-search-prodcom").modal('hide');
      }
    });
  });  
  ...
  // Input QTY or PRICE
  $(".prodcom-form").on("keydown keypress blur focus", "input.qty,input.price", function(){
  ...
  $(".prodcom-form").on("keydown keypress blur focus", "input.qty", function(){
      var itemid = $(this).parent().parent().children(".qtyEntry").children().attr("itemid");
      _gblBindNumericClasses('numeric'); 
      listItems(); 
  })
  ...
  $(".prodcom-form").on("click", "button.removeProduct", function(){
  ...
  $(".prodcom-form").submit(function (e) {
      e.preventDefault();
      swal.fire({
          title: 'Do you want to save production component?',
          type: 'question',
          ...
          if(result.value) {
            ...
            rawwithdraw.append("operatedby", operatedby);
            rawwithdraw.append("proddate", proddate);
            rawwithdraw.append("prodnumber", prodnumber);
            rawwithdraw.append("shift", shift);
            rawwithdraw.append("prodstatus", prodstatus);
            rawwithdraw.append("remarks", remarks);
            rawwithdraw.append("postedby", postedby);
            rawwithdraw.append("productlist", productlist);
           
            $.ajax({
               url:"ajax/prodcom_save_record.ajax.php",
               method: "POST",
               data: rawwithdraw,
               cache: false,
               contentType: false,
               ...
               complete: function () {
                swal.fire({
                    title: 'Product component successfully saved!',
                    type: 'success',
                    allowOutsideClick: false,
                ...
                initialize();
                //  swal.fire({
                //     title: 'Production component successfully saved!',
                //     type: 'success',
   ...
   $("#btn-cancel").click(function(){
      swal.fire({
          title: 'Do you want to Cancel subcomponents production transaction?',
          ...
          if(result.value) {
            ...
            $.ajax({
               url:"ajax/prodcom_cancel_record.ajax.php",
               method: "POST",
               data: cancelprodfin,
               cache: false,
               contentType: false,
