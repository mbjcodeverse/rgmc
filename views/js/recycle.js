...
if (!$.fn.DataTable.isDataTable('.recycleTransactionTable')) {
  var pt = $('.recycleTransactionTable').DataTable({
      deferRender: true,
      processing: true,
      autoWidth: true,
      scrollY: 360,
...
$(function() {
 ...
 // When browsing Purchase Order - set Status to Pending | Partial
  ...
  $("#btn-search").click(function(){
    $(".recycleTransactionTable").DataTable().clear();
    pt.draw();
  ...
  $('#lst-machineid, #lst_date_range, #lst-empid, #lst-recstatus').on("change", function(){
    ...
    $.ajax({
        url:"ajax/recycle_transaction_list.ajax.php",   
        method: "POST",                
        data: data,                    
        cache: false,                  
        contentType: false,            
        ...
        success:function(answer){
              ...
              for(var i = 0; i < answer.length; i++) {
                ...
                let sw = answer[i];
                let req_date = sw.recdate;
                let recdate = req_date.split("-");
                recdate = recdate[1] + "/" + recdate[2] + "/" + recdate[0];

                let requestor = sw.request_by;
                let recnumber = sw.recnumber;
                let shift = sw.shift;
                let machinedesc = sw.machinedesc;
                let machabbr = sw.machabbr;
                let recstatus = sw.recstatus;
                ...
                var button = "<td><button type='button' class='btn btn-outline btn-sm bg-green-400 border-green-400 text-green-400 btn-icon rounded-round border-2 ml-2 btnEditRawout' recnumber='"+recnumber+"'><i class='icon-pencil3'></i></button></td>";  
                pt.row.add([recdate, requestor, recnumber, shift, machine, total_amount, button]); 
              }
              pt.draw();
  ...
  $(".recycleTransactionTable tbody").on('click', '.btnEditRawout', function () {
    var recnumber = $(this).attr("recnumber");
    var data = new FormData();
    data.append("recnumber", recnumber);
    $.ajax({
      url:"ajax/recycle_get_record.ajax.php",
      method: "POST",
      data: data,
      cache: false,
      contentType: false,
      ...
      dataType:"json",
      success:function(answer){
          $("#sel-recycleby").val(answer["recycleby"]).trigger('change');

          let recdate = answer["recdate"].split("-");
          recdate = recdate[1] + "/" + recdate[2] + "/" + recdate[0];
          if (recdate == '00/00/0000'){      
            recdate = '';
          }
          $("#date-recdate").val(recdate);
          $("#txt-recstatus").val(answer["recstatus"]);
          $("#txt-recnumber").val(answer["recnumber"]);
          ...
          data_items.append("recnumber", recnumber);
          $.ajax({
            url:"ajax/recycle_get_items.ajax.php",
            method: "POST",
            data: data_items,
            cache: false,
            contentType: false,
          ...
          let recstatus = $("#txt-recstatus").val();
          switch (recstatus){
            case 'Cancelled':
          ...
          $("#modal-search-recycle").modal('hide');
      }
    });
  });  
  ...
  // Input QTY or PRICE
  $(".recycle-form").on("keydown keypress blur focus", "input.qty,input.price", function(){
  ...
  $(".recycle-form").on("keydown keypress blur focus", "input.qty", function(){
      var itemid = $(this).parent().parent().children(".qtyEntry").children().attr("itemid");
      _gblBindNumericClasses('numeric'); 
      listItems(); 
  })
  ...
  $(".recycle-form").on("click", "button.removeProduct", function(){
  ...
  function initialize(){
    $("#sel-machineid").val('').trigger('change');
    $("#sel-recycleby").val('').trigger('change');
    $("#txt-recnumber").val("");
    $("#txt-recstatus").val("Posted");
    $("#tns-remarks").val("");
    $("#num-amount").val("0.00");
  ...
  $(".recycle-form").submit(function (e) {
      e.preventDefault();
      swal.fire({
          title: 'Do you want to save recycled material?',
          type: 'question',
          ...
          if(result.value) {
            ...
            let recycleby = $("#sel-recycleby").val();

            let format_recdate = $("#date-recdate").val().split("/");
            format_recdate = format_recdate[2] + "-" + format_recdate[0] + "-" + format_recdate[1];
            
            let recdate = format_recdate;
            let recnumber = $("#txt-recnumber").val();
            ...
            rawwithdraw.append("recycleby", recycleby);
            rawwithdraw.append("recdate", recdate);
            rawwithdraw.append("recnumber", recnumber);
            rawwithdraw.append("shift", shift);
            rawwithdraw.append("recstatus", recstatus);
            rawwithdraw.append("remarks", remarks);
            rawwithdraw.append("postedby", postedby);
            rawwithdraw.append("productlist", productlist);
           
            $.ajax({
               url:"ajax/recycle_save_record.ajax.php",
               method: "POST",
               data: rawwithdraw,
               cache: false,
               contentType: false,
               ...
               complete: function () {
                swal.fire({
                    title: 'Recycling successfully saved!',
                    type: 'success',
                    allowOutsideClick: false,
   ...
   $("#btn-cancel").click(function(){
      swal.fire({
          title: 'Do you want to Cancel recycled raw materials?',
          ...
          if(result.value) {
            var recnumber = $("#txt-recnumber").val();            
            var cancelrecycle = new FormData();
            cancelrecycle.append("recnumber", recnumber);            
            $.ajax({
               url:"ajax/recycle_cancel_record.ajax.php",
               method: "POST",
               data: cancelrecycle,
               cache: false,
               contentType: false,
