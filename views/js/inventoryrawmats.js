...
if (!$.fn.DataTable.isDataTable('.inventoryrawmatTransactionTable')) {
  var pt = $('.inventoryrawmatTransactionTable').DataTable({
      deferRender: true,
      processing: true,
      autoWidth: true,
      scrollY: 360,
...
$(function() {
 ...
 // When browsing Purchase Order - set Status to Pending | Partial
  ...
  $("#btn-search").click(function(){
    $(".inventoryrawmatTransactionTable").DataTable().clear();
    pt.draw();
    $('#lst-invstatus').val('Posted').trigger('change');
  });   
  ...
  $("#lbl-lst-invstatus").click(function(){
      $("#lst-invstatus").val('').trigger('change');
  });        

  // Search Stock Withdrawal - Modal Form dynamic selector
  $('#lst-machineid, #lst_date_range, #lst-empid, #lst-invstatus').on("change", function(){
    ...
    let invstatus = $("#lst-invstatus").val();
    ...
    data.append("invstatus", invstatus);

    $.ajax({
        url:"ajax/inventoryrawmats_transaction_list.ajax.php",   
        method: "POST",                
        data: data,                    
        cache: false,                  
        contentType: false,            
        ...
        success:function(answer){
              ...
              for(var i = 0; i < answer.length; i++) {
                ...
                let sw = answer[i];
                let inv_date = sw.invdate;
                let invdate = inv_date.split("-");
                invdate = invdate[1] + "/" + invdate[2] + "/" + invdate[0];

                let countedby = sw.counted_by;
                let invnumber = sw.invnumber;
                let invstatus = sw.invstatus;
                let total_amount = numberWithCommas(sw.total_amount);

                var button = "<td><button type='button' class='btn btn-outline btn-sm bg-green-400 border-green-400 text-green-400 btn-icon rounded-round border-2 ml-2 btnEditInventory' invnumber='"+invnumber+"'><i class='icon-pencil3'></i></button></td>";  
                pt.row.add([invdate, countedby, invnumber, total_amount, button]); 
              }
              pt.draw();
  ...
  $(".inventoryrawmatTransactionTable tbody").on('click', '.btnEditInventory', function () {
    var invnumber = $(this).attr("invnumber");
    var data = new FormData();
    data.append("invnumber", invnumber);
    $.ajax({
      url:"ajax/inventoryrawmat_get_record.ajax.php",
      method: "POST",
      data: data,
      cache: false,
      contentType: false,
      ...
      dataType:"json",
      success:function(answer){
          $("#sel-countedby").val(answer["countedby"]).trigger('change');

          let invdate = answer["invdate"].split("-");
          invdate = invdate[1] + "/" + invdate[2] + "/" + invdate[0];
          if (invdate == '00/00/0000'){      
            invdate = '';
          }
          $("#date-invdate").val(invdate);
          $("#txt-invstatus").val(answer["invstatus"]);
          $("#txt-invnumber").val(answer["invnumber"]);
          ...
          data_items.append("invnumber", invnumber);
          $.ajax({
            url:"ajax/inventoryrawmat_get_items.ajax.php",
            method: "POST",
            data: data_items,
            cache: false,
            contentType: false,
          ...
          $("#modal-search-inventoryrawmat").modal('hide');
      }
    });
  });  
     ...
     $.ajax({
      url:"ajax/get_rawmats_details.ajax.php", 
        ...
        success:function(answer){
          ...
          $("#tns-search").val("");  // Clear the tns-search input field
          filterProducts();  // Call the filter function to show all products

          // Sort the product list by product description (pdesc)
          sortProductListByPdesc();  // Call the function to sort the rows

          //$('.qty').focus();  // Focus on the quantity input of the new row

          // Focus on the quantity input of the newly added product
          setTimeout(function() {
              $('.qty[itemid="'+itemid+'"]').focus().select();
          }, 50); // Slight delay ensures sorting has completed
          // $('.qty').focus();
        }
     })
  });
  ...
  // Input QTY or PRICE
  $(".inventoryrawmat-form").on("keydown keypress blur focus", "input.qty,input.price", function(){
  ...
  $(".inventoryrawmat-form").on("keydown keypress blur focus", "input.qty", function(){
      var itemid = $(this).parent().parent().children(".qtyEntry").children().attr("itemid");
      _gblBindNumericClasses('numeric'); 
      listItems(); 
  })
  ...
  $(".inventoryrawmat-form").on("click", "button.removeProduct", function(){
  ...
  function initialize(){
    $("#sel-countedby").val('').trigger('change');
    $("#date-invdate").val("");
    $("#txt-invnumber").val("");
    $("#txt-invstatus").val("Posted");
    $("#tns-remarks").val("");
  ...
  $(".inventoryrawmat-form").submit(function (e) {
      e.preventDefault();
      swal.fire({
          title: 'Do you want to save raw material inventory?',
          type: 'question',
          ...
          if(result.value) {
            ...
            let invnumber = $("#txt-invnumber").val();
            let invstatus = $("#txt-invstatus").val();
            let remarks = $("#tns-remarks").val();
            let postedby = $("#tns-postedby").val();
            let productlist = $("#productList").val();           

            var rawinventory = new FormData();
            rawinventory.append("trans_type", trans_type);
            rawinventory.append("countedby", countedby);
            rawinventory.append("invdate", invdate);
            rawinventory.append("invnumber", invnumber);
            rawinventory.append("invstatus", invstatus);
            rawinventory.append("remarks", remarks);
            rawinventory.append("postedby", postedby);
            rawinventory.append("productlist", productlist);
           
            $.ajax({
               url:"ajax/inventoryrawmat_save_record.ajax.php",
               method: "POST",
               data: rawinventory,
               cache: false,
               contentType: false,
               ...
               complete: function () {
                swal.fire({
                    title: 'Raw materials inventory successfully saved!',
                    type: 'success',
                    allowOutsideClick: false,
  ...
  $("#tns-search").on("keyup", function() {
    filterProducts();
  });

  // Function to filter products
  function filterProducts() {
      let keyword = $("#tns-search").val().toLowerCase();  // Get the keyword from the search field
      $(".enlisted_products tr").each(function() {
          let productDesc = $(this).find(".pdesc").val().toLowerCase();  // Get the product description
          
          // Show or hide rows based on whether the product description matches the search keyword
  ...
  // Function to sort the product list by product description (pdesc)
  function sortProductListByPdesc() {
      let rows = $(".enlisted_products tr").get(); // Get all rows in the enlisted_products table

      rows.sort(function(a, b) {
          let descA = $(a).find(".pdesc").val().toLowerCase(); // Get product description of row a
          let descB = $(b).find(".pdesc").val().toLowerCase(); // Get product description of row b
      ...
      // Append the sorted rows back to the enlisted_products table
      $.each(rows, function(index, row) {
          $(".enlisted_products").append(row);
      });
  }
   ...
   $("#btn-cancel").click(function(){
      swal.fire({
          title: 'Do you want to Cancel requested raw materials?',
          ...
          if(result.value) {
            var invnumber = $("#txt-invnumber").val();           
            var cancelrequest = new FormData();
            cancelrequest.append("invnumber", invnumber);            
            $.ajax({
               url:"ajax/inventoryrawmat_cancel_record.ajax.php",
               method: "POST",
               data: cancelrequest,
               cache: false,
               contentType: false,
