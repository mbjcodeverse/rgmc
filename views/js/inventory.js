...
if (!$.fn.DataTable.isDataTable('.inventoryTransactionTable')) {
var pt = $('.inventoryTransactionTable').DataTable({
    deferRender: true,
    processing: true,
    autoWidth: true,
    scrollY: 360,
...
$(function() {
  ...
  $("#btn-search").click(function(){
    $(".inventoryTransactionTable").DataTable().clear();
    pt.draw();
  ...
  // Search Physical Inventory - Modal Form dynamic selector
  $('#lst_date_range, #lst-empid, #lst-invstatus').on("change", function(){
    ...
    $.ajax({
        url:"ajax/inventory_transaction_list.ajax.php",   
        method: "POST",                
        data: data,                    
        cache: false,                  
        contentType: false,            
        ...
        success:function(answer){
              ...
              for(var i = 0; i < answer.length; i++) {
                ...
                pt.row.add([invdate, countedby, invnumber, invstatus, total_amount, button]); 
              }
              pt.draw();
  ...
  $(".inventoryTransactionTable tbody").on('click', '.btnEditInventory', function () {
    var invnumber = $(this).attr("invnumber");
    var data = new FormData();
    data.append("invnumber", invnumber);
    $.ajax({
      url:"ajax/inventory_get_record.ajax.php",
      method: "POST",
      data: data,
      cache: false,
      contentType: false,
      ...
      success:function(answer){
          ...
          $.ajax({
            url:"ajax/inventory_get_items.ajax.php",
            method: "POST",
            data: data_items,
            cache: false,
            contentType: false,
          ...
          $("#modal-search-inventory").modal('hide');
      }
    });
  });  
  ...
  // Input QTY or PRICE
  $(".inventory-form").on("keydown keypress blur focus", "input.qty,input.price", function(){
  ...
  $(".inventory-form").submit(function (e) {
      e.preventDefault();
      swal.fire({
          title: 'Do you want to save physical inventory transaction?',
          type: 'question',
          ...
          if(result.value) {
            ...
            // alert(trans_type + ' ' + countedby + ' ' + invdate + ' ' + invstatus + ' ' + remarks + ' ' + postedby + ' ' + productlist);

            var stockout = new FormData();
            stockout.append("trans_type", trans_type);
            stockout.append("countedby", countedby);
            stockout.append("invdate", invdate);
            stockout.append("invnumber", invnumber);
            stockout.append("invstatus", invstatus);
            ...
            $.ajax({
              ...
              complete: function () {
                swal.fire({
                    title: 'Physical inventory transaction successfully saved!',
                    type: 'success',
  ...
  $("#btn-print").click(function(){
    ...
    window.open("extensions/tcpdf/pdf/inventory.php?invnumber="+invnumber, "_blank");
  });
});    